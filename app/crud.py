...
def count_events(
    db: Session, user_id: str = None, device_id: str = None, event_type: str = None
):
    query = _get_base_query(db, models.Event, user_id, device_id)
    if event_type:
...
def get_badges(db: Session, user_id: str = None, device_id: str = None):
    query = _get_base_query(db, models.Badge, user_id=user_id, device_id=device_id)
    return query.all()


def create_or_update_user_badge(
    db: Session, badge_name: str, level: int, user_id: str = None, device_id: str = None
):
    try:
        query = _get_base_query(db, models.Badge, user_id=user_id, device_id=device_id)
        ...
        db_item = models.Badge(
            user_id=user_id, device_id=device_id, badge_name=badge_name, level=level
        )
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
...
def get_scores(
    ...
    query = _get_base_query(
        db, models.Event, user_id=user_id, device_id=device_id, query=query
    )
    if event_type:
        query = query.filter(models.Event.event_type == event_type)
